...
test("GitHub Pages artifact preserves the production host and static distribution", () => {
  for (const path of ["index.html", "CNAME", ".nojekyll", "distribution/agent.txt", "favicon.svg", "product/home.jpg", "product/calendar.jpg", "art/companyos-sculpture.jpg", "art/companyos-team.jpg", "robots.txt"]) assert.ok(existsSync(`dist/${path}`), `missing ${path}`);
  assert.equal(readFileSync("dist/CNAME", "utf8").trim(), "operaos.ai");
  const html = readFileSync("dist/index.html", "utf8");
  assert.match(html, /CompanyOS \| Your company\. One system\./);
...
test("all three languages include the three-part product story", () => {
  ...
  for (const text of ["Your computer", "Your system", "Your team", "Seu computador", "Seu sistema", "Sua equipe", "Tu computadora", "Tu sistema", "Tu equipo", "OS Agent", "OS Dev", "Bring the work you’ve already done.", "Traga o trabalho que você já fez.", "Trae el trabajo que ya hiciste.", "Automatic backups", "Backups automáticos", "Respaldos automáticos"]) assert.ok(javascript.includes(text), `missing built copy: ${text}`);
});
test("public page uses the CompanyOS identity without obsolete sales paths", () => {
