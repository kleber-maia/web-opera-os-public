    ...
    <main id="content">
      ...
      <section className="system-section wrap section-space" id="system" aria-labelledby="system-title">
        ...
        <div className="system-body" id="modules">
          <div className="module-composition scroll-art"><div className="module-pieces">{copy.modules.items.map(([name], index) => <div className={`module-piece piece-${index}`} key={index}><span>{name}</span><Plus size={19} aria-hidden="true" /></div>)}<div className="module-open"><Plus size={34} aria-hidden="true" /></div></div></div>
          <div className="module-copy"><h3>{copy.modules.title}</h3><p>{copy.modules.body}</p><p className="module-note">{copy.modules.final}</p><figure className="product-proof"><ProductDevices desktop="/product/knowledge-desktop.png" alt={copy.system.alt[0]} phoneAlt={copy.system.alt[1]} /><figcaption>{copy.system.caption}</figcaption></figure></div>
        </div>
        <div className="module-carousel motion-art" data-stopped={modulesPaused} role="region" aria-label={copy.modules.title}>
          <div className="carousel-toolbar"><p>{copy.modules.final}</p><button type="button" onClick={() => setModulesPaused(!modulesPaused)} aria-pressed={modulesPaused} aria-label={modulesPaused ? copy.hero.play : copy.hero.pause}>{modulesPaused ? <Play size={16} weight="fill" /> : <Pause size={16} weight="fill" />}</button></div>
          <div className="module-marquee"><div className="module-track">{[0, 1].map(group => <div className="module-track-group" key={group} aria-hidden={group === 1 ? true : undefined}>{copy.modules.items.map(([name, detail], index) => <article className="module-slide" key={name}><span className="module-slide-number">0{index + 1}</span><h3>{name}</h3><p>{detail}</p></article>)}</div>)}</div></div>
        </div>
